...
export default function ServiceRequestDetailModal({
    ...
    const [currentStatus, setCurrentStatus] = useState('Emergency');
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            {(cancelMessage || quoteStatusMessage) && (
                <div className={`fixed top-4 right-4 z-[70] px-6 py-4 rounded-lg shadow-lg flex items-center gap-3 min-w-[300px] max-w-md ${(cancelMessage?.type === 'success' || quoteStatusMessage?.type === 'success')
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-gray-900">
                                Service Request #{displayData.booking_id || displayData.reference_id || '--'}
                            </h2>
                            <div className="flex items-center gap-2 mt-0.5 text-xs font-medium text-gray-800">
                                <span className="bg-yellow-200 px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wide">
                                    {((displayData as unknown) as { service_type_title?: string; service_name?: string; service_type?: string; [key: string]: unknown }).service_type_title || ((displayData as unknown) as { service_type_title?: string; service_name?: string; service_type?: string; [key: string]: unknown }).service_name || ((displayData as unknown) as { service_type_title?: string; service_name?: string; service_type?: string; [key: string]: unknown }).service_type || 'Service'}
                                </span>
                                <span className="opacity-75">
                                    Created: {new Date((((displayData as unknown) as { created_at?: string; scheduled_at?: string; [key: string]: unknown }).created_at || ((displayData as unknown) as { created_at?: string; scheduled_at?: string; [key: string]: unknown }).scheduled_at || Date.now())).toLocaleString()}
                                </span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    readOnly
                                    className="bg-white border border-gray-200 text-gray-900 pl-3 pr-8 py-1.5 rounded-lg font-medium text-xs shadow-sm outline-none"
                                />
                                <FaCalendarAlt className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none w-3 h-3" />
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-5 bg-gray-50">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 max-w-7xl mx-auto">
                        ...
                        <div className="lg:col-span-2 space-y-4">
                            ...
                            {/* Vehicle Information */}
                            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
                                <h3 className="text-sm font-bold text-gray-900 mb-3">Vehicle Information</h3>
                                <div className="grid grid-cols-2 gap-y-4 gap-x-8">
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Make & Model</label>
                                    ...
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">VIN</label>
                                        <p className="font-bold text-gray-900 text-xs">
                                            {(() => {
                                    ...
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">License Plate</label>
                                    ...
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Mileage</label>
                                        <p className="font-bold text-gray-900 text-xs">
                                            {(() => {
                                    ...
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Color</label>
                                        <p className="font-bold text-gray-900 text-xs">
                                            {(() => {
                                    ...
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Engine</label>
                                        <p className="font-bold text-gray-900 text-xs">
                                            {(() => {
                            ...
                            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
                                ...
                                <div className="grid grid-cols-2 gap-y-4 gap-x-8">
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Service Type</label>
                                        <p className="font-bold text-gray-900 text-xs">
                                            {(() => {
                                    ...
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Destination</label>
                                        <p className="font-bold text-gray-900 text-xs">
                                            {(() => {
                                                ...
                                                return data.dropoff_location?.name || data.dropoff_address || 'AutoService Center';
                                            })()}
                                        </p>
                                    </div>
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Requested Date/Time</label>
                                        <p className="font-bold text-gray-900 text-xs">
                                            {(() => {
                                    ...
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Distance</label>
                                        <p className="font-bold text-gray-900 text-xs">8.5 miles</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 h-full flex flex-col">
                                ...
                                <div className="space-y-3 flex-1">
                                    ...
                                    <div className="flex justify-between text-xs py-1.5 border-b border-dashed border-gray-100">
                                        <span className="text-gray-600">Base Tow Service</span>
                                    ...
                                    <div className="flex justify-between text-xs py-1.5 border-b border-dashed border-gray-100">
                                        <span className="text-gray-600">Transportation fees</span>
                                    ...
                                    <div className="flex justify-between items-center pt-3 mt-3 border-t border-gray-200">
                                        <span className="font-bold text-gray-900 text-xs">Total Amount</span>
